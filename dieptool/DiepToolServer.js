...
const DiepToolServer = (server) => {
    const WebSocket = require('ws');
    const wss = new WebSocket.Server({ server });
    new DiepToolManager(wss);
};

class DiepToolManager {
    constructor(wss) {
        this.users = new Set();
        ...
        wss.on('connection', (ws, req) => {
            ...
            ws.once('login', (authToken) => this.onLoginHandler(ws, authToken));
            ws.once('close', () => this.ips.delete(ip));
            ws.once('error', (err) => {});
        });
